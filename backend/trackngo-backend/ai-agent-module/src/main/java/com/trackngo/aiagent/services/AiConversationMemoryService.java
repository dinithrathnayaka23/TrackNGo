...
public class AiConversationMemoryService {
    ...
    public String recentConversationDigest(String chatId, int limit) {
        ...
        try {
            // Scoped to the signed-in account as well as the conversation. A chat id is
            // generated by the client, so nothing stops two clients producing the same
            // one; without this, whoever collided would read the other's history.
            AgentExecutionContext.Context context = AgentExecutionContext.get();
            Long userId = context == null ? null : context.userId();
            List<String> rows = jdbc.query("""
                    SELECT role, content
