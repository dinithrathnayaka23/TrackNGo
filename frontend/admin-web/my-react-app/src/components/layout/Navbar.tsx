...
function Navbar({
  ...
  return (
    ...
    >
      <div className="flex flex-nowrap items-center gap-3 text-sm text-[#64748b] whitespace-nowrap">
        {breadcrumbs.map((item, index) => {
          const isLast = index === breadcrumbs.length - 1
          return (
            <div key={`${item}-${index}`} className="flex items-center gap-3">
              <span className={isLast ? 'whitespace-nowrap font-bold text-[#111827]' : 'whitespace-nowrap'}>{item}</span>
      ...
      <div className="relative flex items-center gap-8">
        ...
        <button
          ...
          className="relative text-sm text-[#334155] transition duration-200 hover:scale-105"
