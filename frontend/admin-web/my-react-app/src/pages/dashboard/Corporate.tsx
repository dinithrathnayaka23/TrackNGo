...
function Corporate() {
  ...
  return (
    <section className="mx-auto w-full max-w-[1340px]">
      ...
      <header className="mb-6 flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-xl font-extrabold tracking-tight text-[#111827]">Corporate Accounts</h1><p className="mt-1 text-sm text-[#64748b]">Live corporate accounts, contracts, and billing totals from TrackNGo.</p></div><div className="flex flex-wrap items-center gap-2"><button type="button" onClick={() => navigate('/dashboard/corporate/contracts')} className="inline-flex h-10 items-center gap-2 rounded-xl bg-[#2642a6] px-4 text-sm font-semibold text-white transition hover:bg-[#203b96]">Contract Requests</button><button type="button" onClick={() => navigate('/dashboard/corporate/pricing-settings')} className="inline-flex h-10 items-center gap-2 rounded-xl border border-[#d6dbe6] bg-white px-4 text-sm font-semibold text-[#334155] transition hover:bg-[#f8fafc]">Pricing Settings</button></div></header>
      <div className="mb-5 grid gap-4 md:grid-cols-3"><article className="rounded-2xl border border-[#e5e7eb] bg-white px-5 py-4"><p className="text-sm font-semibold text-[#64748b]">Total Clients</p><p className="mt-1 text-xl font-extrabold text-[#111827]"><FontAwesomeIcon icon={faBuildingUser} className="mr-2 text-[#64748b]" />{clients.length}</p></article><article className="rounded-2xl border border-[#e5e7eb] bg-white px-5 py-4"><p className="text-sm font-semibold text-[#64748b]">Contract Revenue</p><p className="mt-1 text-xl font-extrabold text-[#0f766e]"><FontAwesomeIcon icon={faMoneyBillWave} className="mr-2 text-[#64748b]" />{formatRevenue(totalRevenue)}</p></article><article className="rounded-2xl border border-[#e5e7eb] bg-white px-5 py-4"><p className="text-sm font-semibold text-[#64748b]">Pending Verifications</p><p className="mt-1 text-xl font-extrabold text-[#111827]"><FontAwesomeIcon icon={faClipboardCheck} className="mr-2 text-[#64748b]" />{pending}</p></article></div>
      <article className="mb-5 flex flex-wrap items-center gap-3 rounded-xl border border-[#e5e7eb] bg-white p-5"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search by company, registration no, or manager" className="min-w-[260px] flex-1 rounded-lg border border-[#d6dbe6] px-4 py-2 outline-none focus:ring-2 focus:ring-[#22449d]" /><select value={status} onChange={(event) => setStatus(event.target.value as 'All' | VerificationStatus)} className="rounded-lg border border-[#d6dbe6] bg-white px-4 py-2 text-[#334155]"><option value="All">All Statuses</option><option value="Verified">Verified</option><option value="Pending">Pending</option><option value="Suspended">Suspended</option><option value="Inactive">Inactive</option></select><button type="button" onClick={() => { setQuery(''); setStatus('All') }} className="px-3 py-2 text-sm text-[#64748b]">Clear Filters</button></article>
