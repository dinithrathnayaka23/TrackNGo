...
function ContractCard({ contract }: { contract: CorporateContract }) {
  return <article className="rounded-xl border border-[#e5e7eb] bg-white p-4">
...
function CorporateDetail() {
  ...
  return <section className="mx-auto w-full max-w-[1280px] space-y-5">
    ...
    {activeTab === 'overview' ? <>
      ...
      <article className="rounded-2xl border border-[#e5e7eb] bg-white p-5 sm:p-6"><div className="flex items-center justify-between gap-3"><h2 className="text-base font-bold text-[#111827]">Latest Invoices</h2><button type="button" onClick={() => setActiveTab('billing')} className="text-sm font-semibold text-[#2642a6] hover:text-[#1b357f]">View all</button></div><InvoiceTable invoices={invoices.slice(0, 5)} /></article>
    </> : null}
    ...
    {activeTab === 'billing' ? <article className="rounded-2xl border border-[#e5e7eb] bg-white p-5 sm:p-6"><div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-lg bg-[#e0e7ff] text-[#2642a6]"><FontAwesomeIcon icon={faFileInvoiceDollar} /></div><div><h2 className="text-base font-bold text-[#111827]">Billing History</h2><p className="mt-1 text-sm text-[#64748b]">Invoices loaded from the corporate billing records.</p></div></div><InvoiceTable invoices={invoices} /></article> : null}
  </section>
}
