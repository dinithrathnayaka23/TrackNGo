...
import {
  ...
  faFloppyDisk, faPen, faPlus, faSearch, faSpinner, faStar, faXmark,
} from '@fortawesome/free-solid-svg-icons'
import {
  createAdminDriver, fetchAdminDrivers, updateAdminDriver,
...
function Driver() {
  ...
  return <section className="mx-auto w-full max-w-7xl space-y-5">
    ...
    {isModalOpen ? <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-[#101426]/50 p-3 sm:items-center sm:p-4"><div className="my-2 flex max-h-[calc(100vh-1.5rem)] w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-[#d8deea] bg-[#f8faff] shadow-[0_28px_80px_rgba(17,27,52,0.32)] sm:my-6 sm:max-h-[calc(100vh-3rem)]"><div className="flex shrink-0 items-center justify-between border-b border-[#e1e5ef] px-4 py-4 sm:px-6"><div className="min-w-0 pr-3"><h2 className="text-lg font-extrabold text-[#1f2937]">{editingId ? 'Edit driver' : 'Create driver account'}</h2><p className="mt-1 text-xs text-[#64748b]">These values are stored in the driver account used by the mobile app.</p></div><button type="button" onClick={() => setIsModalOpen(false)} className="grid h-9 w-9 shrink-0 place-items-center rounded-lg text-[#64748b] hover:bg-white" aria-label="Close driver form"><FontAwesomeIcon icon={faXmark} /></button></div><div className="min-h-0 flex-1 overflow-y-auto"><div className="grid gap-4 px-4 py-5 sm:px-6 md:grid-cols-2"><label className="text-sm font-semibold text-[#45516b]">First name<input value={draft.firstName} onChange={(event) => updateField('firstName', event.target.value)} className="form-input" /></label><label className="text-sm font-semibold text-[#45516b]">Last name<input value={draft.lastName} onChange={(event) => updateField('lastName', event.target.value)} className="form-input" /></label><label className="text-sm font-semibold text-[#45516b]">Email<input type="email" value={draft.email} onChange={(event) => updateField('email', event.target.value)} className="form-input" /></label><label className="text-sm font-semibold text-[#45516b]">{editingId ? 'Reset password (optional)' : 'Initial password'}<input type="password" value={draft.password ?? ''} onChange={(event) => updateField('password', event.target.value)} className="form-input" placeholder={editingId ? 'Leave blank to keep current' : ''} /></label><label className="text-sm font-semibold text-[#45516b]">Phone number<input inputMode="numeric" maxLength={10} value={draft.phoneNumber} onChange={(event) => updateField('phoneNumber', event.target.value.replace(/\D/g, '').slice(0, 10))} className="form-input" placeholder="0XXXXXXXXX" /></label><label className="text-sm font-semibold text-[#45516b]">License number<input maxLength={8} value={draft.licenseNumber} onChange={(event) => updateField('licenseNumber', event.target.value.toUpperCase().replace(/[^B0-9]/g, '').slice(0, 8))} className="form-input" placeholder="B1234567" /></label><label className="text-sm font-semibold text-[#45516b]">License expiry<input type="date" value={draft.licenceExpiry} onChange={(event) => updateField('licenceExpiry', event.target.value)} className="form-input" /></label><label className="text-sm font-semibold text-[#45516b]">Joined date<input type="date" value={draft.joinedDate} onChange={(event) => updateField('joinedDate', event.target.value)} className="form-input" /></label><label className="text-sm font-semibold text-[#45516b]">Years of experience<input type="number" min={0} value={draft.yearsOfExperience} onChange={(event) => updateField('yearsOfExperience', Number(event.target.value))} className="form-input" /></label><label className="text-sm font-semibold text-[#45516b]">Status<select value={draft.status} onChange={(event) => updateField('status', event.target.value)} className="form-input"><option value="active">Active</option><option value="on_leave">On leave</option><option value="suspended">Suspended</option><option value="inactive">Inactive</option></select></label><label className="text-sm font-semibold text-[#45516b]">Bank account number<input value={draft.accountNumber} onChange={(event) => updateField('accountNumber', event.target.value)} className="form-input" /></label><label className="text-sm font-semibold text-[#45516b]">Bank name<input value={draft.bankName} onChange={(event) => updateField('bankName', event.target.value)} className="form-input" /></label><label className="inline-flex items-center gap-2 text-sm font-semibold text-[#45516b]"><input type="checkbox" checked={draft.isVerified} onChange={(event) => updateField('isVerified', event.target.checked)} className="h-4 w-4 accent-[#2642a6]" /> License verified</label><label className="inline-flex items-center gap-2 text-sm font-semibold text-[#45516b]"><input type="checkbox" checked={draft.isPhoneVerified} onChange={(event) => updateField('isPhoneVerified', event.target.checked)} className="h-4 w-4 accent-[#2642a6]" /> Phone verified</label></div></div><div className="flex shrink-0 flex-wrap items-center justify-end gap-3 border-t border-[#e1e5ef] px-4 py-4 sm:px-6">{formError ? <p className="mr-auto min-w-0 basis-full text-sm font-semibold text-[#d14343] sm:basis-auto">{formError}</p> : null}<button type="button" onClick={() => setIsModalOpen(false)} className="rounded-lg border border-[#d6dbe6] bg-white px-5 py-2 text-sm font-bold text-[#334155]">Cancel</button><button type="button" onClick={() => void saveDriver()} disabled={saving} className="inline-flex items-center gap-2 rounded-lg bg-[#2642a6] px-5 py-2 text-sm font-bold text-white disabled:opacity-60"><FontAwesomeIcon icon={saving ? faSpinner : faFloppyDisk} className={saving ? 'animate-spin' : ''} />{saving ? 'Saving...' : editingId ? 'Save changes' : 'Create driver'}</button></div></div></div> : null}
  </section>
}
