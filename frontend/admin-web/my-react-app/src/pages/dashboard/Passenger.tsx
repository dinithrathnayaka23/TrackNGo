    ...
    <section className="mx-auto w-full max-w-[1320px]">
      ...
      <article className="rounded-2xl border border-[#dfe4ef] bg-white p-4">
        <div className="mb-4 flex flex-wrap items-center gap-3">
          <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search by name, email, or phone" className="min-w-[260px] flex-1 rounded-lg border border-[#d9dde5] px-4 py-2 outline-none focus:ring-2 focus:ring-[#22449d]" />
          <select value={status} onChange={(event) => setStatus(event.target.value as StatusFilter)} className="rounded-lg border border-[#d9dde5] bg-white px-4 py-2 text-[#334155]"><option value="All">All Status</option><option value="Active">Active</option><option value="Suspended">Suspended</option><option value="Inactive">Inactive</option></select>
          <input type="date" value={registeredDate} onChange={(event) => setRegisteredDate(event.target.value)} className="rounded-lg border border-[#d9dde5] px-4 py-2" aria-label="Registration date" />
