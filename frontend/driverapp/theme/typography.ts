...
/**
 * TrackNGo driver app type system.
 *
 * Shared verbatim with the passenger app (mobile-app/TrackNgo-Mobile/theme/
 * typography.ts) so a driver screen and a passenger screen render the same
 * element at the same size. The two apps are separate Expo packages with no
 * shared workspace, so this file is duplicated rather than imported; keep the
 * two copies in sync when the scale changes.
 *
 * Use these tokens for any new text instead of raw numbers.
 */
