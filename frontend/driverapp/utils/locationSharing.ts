...
/**
 * Records what was chosen in the device's location dialog at sign-in. The dashboard
 * reads it to decide whether to publish the bus position, so both screens share
 * this one key.
 */
...
export async function readShareLocation(): Promise<boolean> {
  try {
    // Absent means the dialog has not been answered on this device yet. The dashboard
    // still asks the OS for permission of its own, so nothing is published before
    // someone has agreed to it.
    return (await AsyncStorage.getItem(DRIVER_SHARE_LOCATION_KEY)) !== "false";
  } catch (error) {
...
export async function rememberShareLocation(enabled: boolean): Promise<void> {
  try {
    await AsyncStorage.setItem(DRIVER_SHARE_LOCATION_KEY, String(enabled));
  } catch (error) {
...
/**
 * Run at sign-in, at the start of a shift. The device's own dialog is the whole
 * prompt - no in-app alert warms it up and none follows it - so the question is asked
 * once, by the platform, with its own explanation and its own link to Settings.
